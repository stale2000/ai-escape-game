...
// Try to "turn off" all "lights" at the same time.
// It is not easy :-)
...
for (var index = 0; index < pattern.length; index++) {
  var entry = pattern[index];
  if (entry == 0) {
    var [x, y] = gg.toXY(index);
    gg.toggleButton(x,y);
...
// alternative: 
// pattern.forEach((e,i)=>{if(!e)gg.toggleButton(...gg.toXY(i))});
...
export default function AgentsPage() {
  ...
  const InfoCard = (
   ...
   /*
  useEffect(() => {
    const parentElement = document.getElementById('grid-game-container') as HTMLElement;
    const gg = new GridGame(6, 5, undefined, parentElement);
...
gg.onClick(function click(x, y) {
  ...
  }, []); // This useEffect already runs only once due to the empty dependency array.
*/
  ...
  return (
    <div>
       <Snake />
      <div>Random Text: {Math.random().toString(36).substring(7)}</div>
      <div id="grid-game-container"></div>
    </div>
  );
  /*
  return (
